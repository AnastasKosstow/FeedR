...
app.MapPost("/pricing/start", async (PricingRequestsChannel channel, IPricingRequestFactory pricingFactory) =>
{
    await channel.Requests.Writer.WriteAsync(pricingFactory.StartRequest());
    return Results.Ok;
});

app.MapPost("/pricing/stop", async (PricingRequestsChannel channel, IPricingRequestFactory pricingFactory) =>
{
